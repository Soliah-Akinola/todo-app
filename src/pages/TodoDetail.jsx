...
function TodoDetail() {
  ...
  if (isLoading) {
    return <div className="text-center text-slate-500 mt-10">Loading task...</div>;
  }

  if (isError || !todo) {
    return <div className="text-center text-red-500 mt-10">Failed to load task.</div>;
  }
  ...
  return (
    <section className="w-screen min-h-screen bg-gradient-to-br from-slate-100 to-slate-200 py-16 px-6">
      <div className="max-w-xl mx-auto bg-white shadow-xl rounded-3xl p-6 border border-slate-300">
        ...
        <p className="text-lg text-slate-700 mb-4">
          <span className="font-semibold">Description:</span> {todo.description || 'No description provided.'}
        </p>
        <p className="text-lg text-slate-700">
          <span className="font-semibold">Status:</span>{' '}
          <span className={todo.completed ? 'text-green-600' : 'text-orange-500'}>
